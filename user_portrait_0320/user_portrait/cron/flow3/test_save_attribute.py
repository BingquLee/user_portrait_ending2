...
from global_utils import retweet_redis_dict, comment_redis_dict, R_SPARK
...
def save_retweet(uid, direct_uid, timestamp):
    ...
    R_SPARK.hincrby('retweet_'+str(uid), str(direct_uid), 1)
    R_SPARK.hincrby('be_retweet_'+str(direct_uid), str(uid), 1)
