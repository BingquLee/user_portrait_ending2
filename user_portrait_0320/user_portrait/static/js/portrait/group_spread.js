  ...
  Draw_table:function(data){
   ...
   if(influ_his[1][1] == 0 & influ_his[1][5] == 1){
    ...
    if(topics.length == 0){
        ...
        Draw_topic_group_spread(topics,'influence_topic', 'influ_topic_WordList','showmore_topic_influ', '领域','人次');
    };
    if(domains.length == 0){
    ...
    //console.log(domains);
        Draw_topic_group_spread(domains,'influence_domain', 'influ_domain_WordList','showmore_domain_influ','身份' ,'人数');
    }
    ...
    if(influ_out['out_friendsnum_his'][1][1] == 0 & influ_out['out_friendsnum_his'][1][5] == 1){
        ...
        draw_friends_distribution(influ_out['out_friendsnum_his'],'group_friends_distribution', '关注数');
    };
...
function Draw_group_influ_weibo(data, div_name, sub_div_name){
    ...
    if (data.length < page_num) {
        ...
        if (data.length == 0) {
            $('#group_spread_loading').css('display', 'none');
            $('#' + sub_div_name).empty();
...
function page_group_influ_weibo(start_row,end_row,data, sub_div_name){
    ...
    $('#group_spread_loading').css('display', 'none');
    $('#group_influ_weibo_result').css('display', 'block');
}
...
function submit_date_user(){
    $('#group_influ_weibo_result').css('dispaly', 'none');
    $('#group_spread_loading').css('display','block');
...
function get_radar_data_spread(data){
 // console.log('gdfzbdf',data);
  var topic = data;
  var topic_name = [];
  var topic_value = [];
  for(var i=0; i<topic[0].length;i++){
    topic_value.push(topic[1][i]);
...
function Draw_topic_group_spread(data, radar_div, motal_div, show_more , title_a,title_people){
  var topic = [];
  ...
  if(topic_sta.length == 0){
      ...
      html += '<th style="text-align:center">'+title_a+'</th>';
      html += '<th style="text-align:center">'+title_people+'</th></tr>';
      for (var i = 0; i < topic_sta.length; i++) {
         var s = i.toString();
         var m = i + 1;
         html += '<tr style=""><th style="text-align:center">' + m + '</th><th style="text-align:center">'
         //<a href="/index/search_result/?stype=2&uid=&uname=&location=&hashtag=&adkeyword=' + topic_name_sta[i] +  '&psycho_status=&domain&topic" target="_blank">' + topic_name_sta[i] +  '</a>
        html += topic_name_sta[i] +'</th><th style="text-align:center">' + topic_sta[i].toFixed(0) + '</th></tr>';
      };
      html += '</table>'; 
      $('#'+ motal_div).append(html);
    };
    ...
    var topic_result = [];
    topic_result = get_radar_data_spread(topic_val);
    //get_radar_data(topic_val)
...
function g_inf_load(){
var group_influ_url = '/group/show_group_result/?task_name='+name+'&submit_user='+user+'&module=influence';
...
var weibo_user_url = '/group/group_member/?task_name='+name+'&submit_user='+user;
