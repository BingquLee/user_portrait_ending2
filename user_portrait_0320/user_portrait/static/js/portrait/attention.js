...
Draw_attention:function(data){
  var texts = '';
  console.log('ohmygod',data);
      $('#test1').css('display', 'none');
      $('#analyze_block').css('display', 'none');
    $('#out_list_button').css('display','none');
  $('.social_loading').css('display', 'block');
	var items = data;
  ...
  $('#test1').css('display', 'block');
      $('#analyze_block').css('display', 'block');
    //$('#out_list_button').css('display','block');
  $('.social_loading').css('display', 'none');
  }
}
...
function attention(data,UserID,UserName,texts){
    ...
    for (var i=0;i<out_data.length;i++){
            ...
            if(out_data[i][1]=='unknown'||out_data[i][1]=='未知'||out_data[i][1]==''){
              //rename_out = '未知';
              nod['name'] = nod_name_out;
              nod['label'] = nod_name_out;
              nod['uid'] = nod_name_out;
            }else{
    ...
    for (var i=0;i<in_data.length;i++){
      ...
      if(in_data[i][1]=='unknown'||in_data[i][1]==''){
        //var rename_in = '未知';
        ...
        function(ec){
            ...
            function focus(param) {
                ...
                if (
                    ...
                    var ajax_url ;
                    if(data.category == 1){
                        ajax_url = '/attribute/identify_uid/?uid='+data.uid;
                        weibo_url = 'http://weibo.com/u/'+ data.uid;
                        node_url = personal_url + data.uid;
                    }else{
                        //console.log(data.uid);
                        //console.log(data.name);
                        ajax_url = '/attribute/identify_uid/?uid='+data.uid; 
                        weibo_url = 'http://weibo.com/u/'+ data.name;
                        node_url = '/index/personal_out/?uid=' + data.uid;
                    }                 
                    $.ajax({
                      url: ajax_url,
                      type: 'GET',
...
function draw_topic(data){
    ...
    //html += '<table class="table table-striped table-bordered bootstrap-datatable datatable responsive">';i
    html += '<tr><th style="text-align:center">排名</th><th style="text-align:center">话题</th><th style="text-align:center">人数</th></tr>';
    var i = 1;
...
function draw_field(data){
    ...
    html += '<table class="table table-striped ">';
    html += '<tr><th style="text-align:center">排名</th><th style="text-align:center">领域</th><th style="text-align:center">人数</th></tr>';
    var i = 1;
    for (var key in datas) {
       html += '<tr><th style="text-align:center">' + i + '</th><th style="text-align:center">' + datas[key][0] + '</th><th style="text-align:center">' + datas[key][1] +  '</th></tr>';
       i = i + 1;
...
function draw_out_list(data){
    ...
    if(data.length==0){
        html+='无未入库用户'; $('#out_list_button').css('display','none');}else{
    $('#out_list_button').css('display','block');
    html += '<table class="table table-striped ">';
    html += '<thead><tr><th style="width:90px;text-align:center;">用户ID</th><th style="width:150px;text-align:center;">昵称</th><th style="text-align:center;width:80px;">'+select_graph+'数</th><th style="text-align:center;">粉丝数</th><th style="text-align:center;">注册地</th><th style="text-align:center;">关注数</th><th style="text-align:center;">微博数</th><th style="text-align:center;">' + '<input name="out_choose_all" id="out_choose_all" type="checkbox" value="" onclick="out_choose_all()" />' + '</th></tr></thead>';
    html += '<tbody>';
    for(var i = 0; i<data.length;i++){
      ...
      if(item[0]=='None'){
        ...
        html += '<td style="text-align:center" style="width:100px;">'+ item[4] +'</td>';
        html += '<td style="text-align:center" style="width:100px;">'+ item[5] +'</td>';
        html += '<td style="text-align:center" style="width:100px;">'+ item[6] +'</td>';
...
function draw_in_list(data){
  console.log('a',personalData);
  //console.log(uname);
    var select_graph = $('input[name="graph-type"]:checked').attr("title");
    if(personalData.nick_name == 'unknown'||personalData.nick_name == ''){
      var uname = uid;
    }else if(personalData.nick_name == 'undefined'){
      var uname = uid;
    }else{var uname = personalData.nick_name};
    $('#in_list').empty();
    var html = '';
    html += '<table class="table table-striped ">';
    html += '<thead><tr><th  style="/*width:90px;*/text-align:center;">用户ID</th><th style="/*width:150px;*/text-align:center;">昵称</th><th style="text-align:center;">影响力</th><th style="text-align:center;">身份敏感度</th><th style="text-align:center;">'+select_graph+'数</th><th style="text-align:center;">活跃度</th><th style="text-align:center;">言论敏感度</th><th style="text-align:center;">' + '<input name="in_choose_all" id="in_choose_all" type="checkbox" value="" onclick="in_choose_all()" />' + '</th></tr></thead>';
    html += '<tbody>';
    ...
    for(var i = 0; i<data.length;i++){
      ...
      if(item[1] == 'unknown'||item[1] == ''){
        var user_name = item[0];
     ...
     //console.log(item[1]);
      ...
      html += '<td style="text-align:center" style="width:70px;">'+ item[5].toFixed(2) +'</td>';
      html += '<td style="text-align:center" style="width:70px;">'+ item[6].toFixed(2) +'</td>';
...
function in_list_button(){
  ...
  var job = {"task_name":group_name,"task_max_count":p_task_max_count,"submit_user":$('#d_useremail').text(), "uid_list":group_confirm_uids, "state":remark};
  console.log(group_ajax_url);
  $.ajax({
      type:'POST',
      url: group_ajax_url,
