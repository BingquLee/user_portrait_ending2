...
function Draw_top_location(data){
	...
	//console.log('rrr',bar_data_x);
	//console.log('eee', bar_data_y);
	var bar_data_2 = []
...
function show_activity_track(data){
    ...
    $('#track_user_commit').click(function(){
        var track_type = $('#select_track_addition').val();
        if (track_type == 'select_user'){
        ...
        }else if (track_type == 'select_out'){
                var track_user_id = $('#select_track_weibo_out').val();
                month_process_2('out', track_user_id);
        }else{
                var track_user_id = $('#select_track_weibo_in').val();
                month_process_2('in', track_user_id);
        }
    });
    track_init();
}
...
function month_process_2(direction, city){
    //console.log(global_moving_geo);
    require.config({
        paths: {
            echarts: '/static/js/bmap/js'
        },
    ...
    function (echarts, BMapExtension) {
        ...
        var long_city = city;
        var short_city = city.split('\t').pop();
        geolist.push(short_city);
        valuelist.push(0);
        if (direction == 'out'){
            for (key in global_moving_geo){
                var out_in = key.split('&');
                if (out_in[0] == long_city){
                    if (out_in[1].split('\t')[0] == '中国'){
                        geolist.push(out_in[1].split('\t').pop());
                        valuelist.push(global_moving_geo[key]);
                    }
                }
            }
        }
            ...
            for (key in global_moving_geo){
                var out_in = key.split('&');
                if (out_in[1] == long_city){
                    if (out_in[0].split('\t')[0] == '中国'){
                        geolist.push(out_in[0].split('\t').pop());
                        valuelist.push(global_moving_geo[key]);
                    }
                }
            }
        ...
        function drawline(){
            ...
            if (direction == 'out'){
                for (var i = 1; i < geolist.length; i++){
                    newlinklist.push([{name:short_city},{name:geolist[i], value:valuelist[i]}]);
                }
            }
            else{
                for (var i = 1; i < geolist.length; i++){
                    newlinklist.push([{name:geolist[i]},{name:short_city, value:valuelist[i]}]);
                }
            }
            //console.log(newlinklist);
            //console.log(newgeo);
            //newlinklist = [[{name:'北京'}, {name:'江苏',value:90}],[{name:'北京'}, {name:'青岛',value:90}],[{name:'北京'}, {name:'长沙',value:90}]];
            //console.log(newlinklist);
            var option = {
...
function month_process(data){
    ...
    function (echarts, BMapExtension) {
        ...
        for (var i = 0; i < data.length; i++){
            var time_geo = data[i];
            if (time_geo[1] != ''){
                var city_list = time_geo[1].split('\t');
                if (city_list[0] == '中国'){
                    timelist.push(time_geo[0]);
                    var city_city = city_list.pop();
